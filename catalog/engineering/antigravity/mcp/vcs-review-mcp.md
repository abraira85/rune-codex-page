...
name: vcs-review-mcp
status: experimental
context: "Ported from the Claude vcs-review-mcp config (../../claude/mcp/vcs-review-mcp.md) — not yet used against a real GitHub/GitLab PR."
description: Wires a GitHub/GitLab MCP server into Antigravity so code-reviewer, api-design-reviewer, and dependency-reviewer can post review comments directly on the real PR/MR instead of only returning text.
---

# VCS Review MCP

Gives Antigravity's agents direct, structured access to the actual
pull/merge request (read the diff with full PR context, post inline
comments on specific lines, submit a review with an overall verdict)
through a GitHub/GitLab MCP server, instead of a human copy-pasting
`code-reviewer`'s findings into the PR by hand. Pairs directly with
`code-reviewer`, `api-design-reviewer`, and `dependency-reviewer` in
`../agents/` — once configured, "review this PR" can end with the review
actually posted, not just printed.
...
- This config block goes in `.agents/mcp_config.json` (workspace) or
  ...
  intentionally does not write this for you -- merge it by hand.
...
- Ported from `../../claude/mcp/vcs-review-mcp.md` — same servers, only
