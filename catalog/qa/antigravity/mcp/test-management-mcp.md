...
status: experimental
context: "Ported from the Claude test-management-mcp config (../../claude/mcp/test-management-mcp.md) — not yet used against a real TestRail/Xray/Zephyr/Qase project. Speculative on the MCP-availability front specifically -- verify before relying on it."
description: Wires a dedicated test-case-management platform (TestRail, Xray, Zephyr, Qase, or similar) into Antigravity so test-case-writer's matrix can be synced there and execution results recorded, instead of living only as markdown.
---
...
platform (TestRail, Xray, Zephyr Scale, Qase, or similar) instead of
leaving `test-case-writer`'s matrix as a markdown table nobody re-runs
from.
...
- This config block goes in `.agents/mcp_config.json` (workspace) or
  `~/.gemini/config/mcp_config.json` (global), same as the other MCP
  files in this folder — `scripts/install.py` does not write it for you.
- Ported from `../../claude/mcp/test-management-mcp.md` — the speculative
  caveats there apply equally here; nothing about porting it to
  Antigravity made these servers more confirmed to exist.
