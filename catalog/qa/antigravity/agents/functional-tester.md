...
status: experimental
context: "Ported from the Claude functional-tester agent (../../claude/agents/functional-tester.md) — not yet run against a real feature."
...
This role reads code and specs; it does not execute anything via a
terminal or browser. For anything that needs to actually run (clicking
through a UI, hitting an API), hand off to `browser-automation-tester` or
`api-tester` in this same folder. Ported from the Claude version of this
role with no behavioral changes — see
`../../claude/agents/functional-tester.md`.
