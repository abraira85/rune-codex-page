...
status: experimental
context: "Ported from the Claude test-automation-engineer agent (../../claude/agents/test-automation-engineer.md) — not yet run against a real codebase."
...
5. If a test is flaky and you can't fix the root cause, don't quietly
   ...
   (hand off to `flaky-test-investigator` in this same folder).
...
or `api-tester`. Ported unchanged in substance from
