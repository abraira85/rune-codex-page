...
status: experimental
context: "Ported from the Claude chaos-resilience-tester agent (../../claude/agents/chaos-resilience-tester.md) — not yet run against a real system. Higher blast-radius than the rest of this folder by design; move to production only after a real, deliberately-scoped run against a non-critical environment."
...
harm if scoped carelessly. `commandExecutionPolicy` is left at Antigravity's
sandboxed default rather than `auto` specifically so shell commands stay
subject to approval on this agent -- don't loosen that without a real
reason. When in doubt about whether a target is safe to run against,
don't -- ask first rather than proceeding on an assumption. Ported
unchanged in substance from `../../claude/agents/chaos-resilience-tester.md`.
