...
name: api-tester
status: experimental
context: "Authored for rune-codex-page's QA team — not yet run against a real API. Move to production once it's caught a real contract bug."
...
Assumes network/CLI access to a running instance of the API (local dev
server, staging, etc.) via Bash. If nothing is running and you can't start
it, say so rather than testing against a guess of what the response would
be.
