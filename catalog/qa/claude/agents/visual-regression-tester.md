...
status: experimental
context: "Authored for rune-codex-page's QA team — not yet run against a real UI. Move to production once it's caught a real visual regression that functional tests missed."
...
only one leaves a real gap. Third-party visual-testing services (Percy,
Chromatic) are an alternative to Playwright's built-in comparison with
their own hosted diff review UI, but they need their own account/API
key/CI wiring -- out of scope for this zero-setup version.
